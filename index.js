...
console.log('0x90b02778545671c116232142F43AA896F9EBbFd1'.toLowerCase());
...
// const temp = async () => {
//   console.log('1')
//   // const issue = await util.isTransactionSuccess('0x25cd3c4dd3f6fef1e0a87441d8edfb2a9b6321e755f6ff0231303942d559ab60');
//   const isS = await util.signTransaction(13);
//   console.log(isS);
//   console.log('2')
// }
// temp();
...
app.post("/challenge/:tx_hash&:pkce_challenge", async function (req, response) {
  ...
  const eth_address = await util.isTransactionSuccess(tx_hash);
  ...
  if (eth_address) {
    ...
    const public_key_from_contract = await util.getPublicKeyFromContract(
      eth_address
    );
    ...
    Metadata.findOne(
      ...
      async function (err, metadata) {
        if (metadata) {
          ...
          const isSuccess = await util.signTransaction(numTransaction);
          if (isSuccess) {
            return response.json({
              status: true,
